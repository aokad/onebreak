...
def left_psl_check_for_human(psl_file, key2seq, align_margin = 100): 
    ...
    with open(psl_file, 'r') as hin:
        for line in hin:
            ...
            # T Gap counts
            if int(F[6]) >= 3: continue
            # Q gap counts
            if int(F[4]) >= 3: continue
            # Q start
            if int(F[11]) >= 10: continue
            # mismatch
...
def right_psl_check_for_human(psl_file, key2seq, align_margin = 100): 
    ...
    with open(psl_file, 'r') as hin:
        for line in hin:
            ...
            if int(F[4]) >= 3: continue
            # Q size - Q end
            if (int(F[10]) - int(F[12]) + 1) >= 10: continue
            # mismatch
            if int(F[2]) >= 3: continue
...
def alignment_contig(input_file, contig_file, output_file, reference_genome, blat_option, blat_ooc, virus_db, repeat_db, mitochondria_db, adapter_db):
    
    blat_cmds = ("blat " + blat_option).split(' ')
    if blat_ooc != "":
        blat_cmds.append("-ooc=" + blat_ooc)
